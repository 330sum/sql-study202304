...
-- inner join하면 부서가 00000이면 부서테이블에 00000 번호가 없기 때문에 조회가 안됨
-- 실무) 회원 중 구매이력이 없는 회원 찾을 때 -> outer join 사용
-- 실무) 댓글이 하나도 없는 게시물 찾기, 파일첨부가 안된 게시글 찾기
SELECT 
...
-- ANSI 표준
SELECT 
--    A.emp_no, A.emp_nm, A.dept_cd, B.dept_nm -- 매칭되면 -- 아래와 차이 주의할 것
    A.emp_no, A.emp_nm, B.dept_cd, B.dept_nm -- 매칭안되면 오른쪽 싹다 안나옴 (null)
--*
FROM tb_emp A
left JOIN tb_dept B
...
A.emp_no, A.emp_nm, A.dept_cd, B.dept_nm -- 사원쪽과 매칭이 안되서 dept_cd 하나도 안나옴! (위아래 주의)
*
FROM tb_emp A
...
-- 매칭되는거 41개 + 신입 5명 + 새부서 2개  = 48개 (갯수물어보는 문제 나옴)
-- 오라클에서는 full outer join으로 구현 할 수 없음
SELECT 
    A.emp_no, A.emp_nm, B.dept_cd, B.dept_nm
FROM tb_emp A
